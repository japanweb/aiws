...
function clearFile() {
    ...
    wsl sh -c "echo '#! /bin/bash' > /etc/init.wsl | echo 'systemctl start bt' >> /etc/init.wsl | echo 'systemctl start nginx' >> /etc/init.wsl | echo 'systemctl start mysqld' >> /etc/init.wsl | echo 'systemctl start pure-ftpd' >> /etc/init.wsl | echo 'systemctl start sshd' >> /etc/init.wsl | echo 'systemctl start dbus' >> /etc/init.wsl | echo 'echo \""nameserver 8.8.8.8\"" >> /etc/resolv.conf' >> /etc/init.wsl | echo 'echo \""nameserver 8.8.4.4\"" >> /etc/resolv.conf' >> /etc/init.wsl"
