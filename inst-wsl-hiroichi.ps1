...
$centosFile = "https://hiroichi.co.jp/aws/CentOS7.zip"
...
function clearFile() {
    ...
    wsl sh -c "curl https://hiroichi.co.jp/aws/systemctl.py > /usr/bin/systemctl"
    wsl sh -c "echo '[network]' > /etc/wsl.conf | echo 'generateResolvConf=false' >> /etc/wsl.conf | echo '[automount]' >> /etc/wsl.conf | echo 'enabled = true' >> /etc/wsl.conf | echo 'root = /mnt/' >> /etc/wsl.conf | echo 'options = \""metadata,umask=22,fmask=111\""' >> /etc/wsl.conf | echo 'mountFsTab = true' >> /etc/wsl.conf | echo '[filesystem]' >> /etc/wsl.conf | echo 'umask = 022' >> /etc/wsl.conf"
    ...
    wsl sh -c "echo '#! /bin/bash' > /etc/init.wsl | echo 'systemctl start bt' >> /etc/init.wsl | echo 'systemctl start nginx' >> /etc/init.wsl | echo 'systemctl start mysqld' >> /etc/init.wsl | echo 'systemctl start sshd' >> /etc/init.wsl | echo 'systemctl start dbus' >> /etc/init.wsl | echo 'echo \""nameserver 8.8.8.8\"" > /etc/resolv.conf' >> /etc/init.wsl | echo 'echo \""nameserver 8.8.4.4\"" >> /etc/resolv.conf' >> /etc/init.wsl"
